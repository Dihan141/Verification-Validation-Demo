...
const getUsers = async (req, res) => {
  try {
    const users = await KaifaUser.find(); // fetch all users from the collection
...
const getUserById = async (req, res) => {
  try {
    ...
    const user = await KaifaUser.findOne({ userId: userId }); // find user by userId
...
const addUser = async (req, res) => {
  try {
    ...
    if (!userId || !meterId || !name) {
      return res
        .status(400)
        .json({ message: "userId, meterId, and name are required" });
    }

    // Create new user instance
    const newUser = new KaifaUser({
      userId,
      meterId,
      name,
